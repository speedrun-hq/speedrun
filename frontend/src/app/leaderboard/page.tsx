...
function LeaderboardTable({
  ...
  return (
    <div
      className={`arcade-container ${colorClasses.container} relative group h-full`}
    >
      <div
        className={`absolute inset-0 ${colorClasses.glowBg} blur-sm group-hover:${colorClasses.glowBgHover} transition-all duration-300`}
      />
      <div className="relative">
        <h3
          className={`arcade-text text-lg mb-4 ${colorClasses.headerText} text-center`}
        >
          {chainName}
        </h3>
          ...
          <table className="w-full table-fixed">
            <thead>
              <tr className={`border-b ${colorClasses.headerBorder}`}>
                <th
                  className={`py-2 px-4 text-left arcade-text text-xs ${colorClasses.headerText} w-[80px]`}
                >
                  RANK
                </th>
                <th
                  className={`py-2 px-4 text-left arcade-text text-xs ${colorClasses.headerText} w-[180px]`}
                >
                  SPEEDRUNNER
                </th>
                <th
                  className={`py-2 px-4 text-right arcade-text text-xs ${colorClasses.headerText} w-[100px]`}
                >
                  RUNS
                </th>
                <th
                  className={`py-2 px-4 text-right arcade-text text-xs ${colorClasses.headerText}`}
                >
                  VOLUME
                </th>
              </tr>
                ...
                ? Array.from({ length: 5 }, (_, index) => {
                    ...
                    if (runner) {
                      return (
                        <tr
                          key={index}
                          className={`border-b ${colorClasses.rowBorder} hover:${colorClasses.hoverBg}`}
                        >
                          <td
                            className={`py-3 px-4 arcade-text text-xs ${colorClasses.rowText} truncate`}
                          >
                            #{index + 1}
                          </td>
                          <td
                            className={`py-3 px-4 arcade-text text-xs ${colorClasses.rowText} truncate`}
                          >
                            {truncateAddress(runner.address)}
                          </td>
                          <td
                            className={`py-3 px-4 arcade-text text-xs ${colorClasses.rowText} text-right`}
                          >
                            {runner.total_transfers}
                          </td>
                          <td
                            className={`py-3 px-4 arcade-text text-xs ${colorClasses.rowText} text-right whitespace-nowrap`}
                          >
                            {runner.total_volume}
                          </td>
                        </tr>
                      ...
                      return (
                        <tr
                          key={index}
                          className={`border-b ${colorClasses.rowBorder} opacity-30`}
                        >
                          <td
                            className={`py-3 px-4 arcade-text text-xs ${colorClasses.rowText} truncate`}
                          >
        ...
        Object.entries(chainIds).forEach(([chainName, chainId]) => {
          try {
            ...
            const chainIntents = intents.filter((intent: Intent) => {
              ...
              const destChainId =
                typeof intent.destination_chain === "string"
            ...
            chainIntents.forEach((intent: Intent) => {
              ...
              const getHumanReadableAmount = (
                ...
                const decimals =
                  sourceChain === "56" || sourceChain === 56 ? 18 : 6;
              ...
              if (existingRunner) {
                ...
                const currentVolume = parseFloat(
                  existingRunner.total_volume
                    .replace(" USDC", "")
                    .replace(",", ""),
                );
                const newAmount = getHumanReadableAmount(
                  intent.amount,
                  intent.source_chain,
                );
                ...
                const currentFastest = parseFloat(
                  existingRunner.fastest_time.replace("s", ""),
                );
                ...
                if (
                  new Date(intent.updated_at) >
                  new Date(existingRunner.last_transfer)
                ) {
                  existingRunner.last_transfer = intent.updated_at;
                }

                // Update score based on transfers only
                existingRunner.score = (
                  existingRunner.total_transfers * 10
                ).toString();
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {chains.map((chain, index) => (
              <div key={index} className="relative">
                {errors[chain.name] && (
                  <div
                    className={`absolute -top-6 left-0 right-0 arcade-text text-xs ${chain.colorClasses.headerText} text-center`}
                  >
