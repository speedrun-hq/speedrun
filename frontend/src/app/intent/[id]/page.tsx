...
export default function IntentPage() {
  ...
  useEffect(() => {
    async function fetchIntent() {
      ...
      try {
        ...
        if (intentData.status === 'fulfilled' || intentData.status === 'settled') {
          try {
            const fulfillmentData = await apiService.getFulfillment(id as string);
  ...
  const formatTokenAmount = (amount: string, tokenAddress: string, sourceChain: string) => {
    try {
      ...
      <div className="z-10 max-w-5xl w-full relative">
        <div className="text-center mb-6 md:mb-8">
          <h1 className="arcade-text text-xl md:text-2xl text-primary-500 relative mb-2">
            <span className="absolute inset-0 blur-sm opacity-50">INTENT DETAILS</span>
            INTENT DETAILS
          </h1>
          <Link href="/" className="text-yellow-500 text-xs arcade-text hover:underline">
            ← BACK TO HOME
          </Link>
        </div>
          ...
          <div className="relative p-6">
            {isLoading ? (
              <div className="text-center py-8">
                <p className="arcade-text text-primary-500 animate-pulse">LOADING INTENT DATA...</p>
              </div>
            ) : error ? (
              <div className="text-center py-8">
                <p className="arcade-text text-red-500">ERROR: {error.message}</p>
                <Link href="/" className="arcade-text text-yellow-500 text-sm mt-4 inline-block hover:underline">
                  RETURN HOME
                </Link>
              </div>
            ) : intent ? (
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-4">
                    {intent.id && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">INTENT ID</h3>
                    ...
                    {intent.status && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">STATUS</h3>
                        <p className={`arcade-text ${
                          intent.status === 'pending' ? 'text-yellow-300' :
                          intent.status === 'fulfilled' ? 'text-green-500' :
                          intent.status === 'settled' ? 'text-green-500' : 'text-gray-300'
                        }`}>
                    ...
                    {intent.source_chain && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">FROM CHAIN</h3>
                        <p className="arcade-text text-gray-300">{getChainName(intent.source_chain)}</p>
                      </div>
                    )}
                    
                    {intent.destination_chain && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">TO CHAIN</h3>
                        <p className="arcade-text text-gray-300">{getChainName(intent.destination_chain)}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    {intent.token && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">TOKEN</h3>
                        <p className="arcade-text text-gray-300">{getTokenSymbol(intent.token, intent.source_chain)}</p>
                      </div>
                    )}
                    
                    {intent.amount && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">AMOUNT</h3>
                        <p className="arcade-text text-gray-300">{formatTokenAmount(intent.amount, intent.token, intent.source_chain)}</p>
                      </div>
                    )}
                    
                    {intent.recipient && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">RECIPIENT</h3>
                        <p className="arcade-text text-gray-300">
                          {truncateText(intent.recipient, 10, 8)}
                          <button 
                            onClick={() => navigator.clipboard.writeText(intent.recipient)}
                            className="ml-2 text-primary-500 text-xs hover:text-primary-400"
                    ...
                    {intent.intent_fee && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">FEE</h3>
                        <p className="arcade-text text-gray-300">{formatTokenAmount(intent.intent_fee, intent.token, intent.source_chain)}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {fulfillment && (
                  <div className="border-t border-gray-700 pt-4 mt-4">
                    <h3 className="arcade-text text-green-500 text-sm mb-3">FULFILLMENT DETAILS</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {fulfillment.fulfiller && (
                        <div>
                          <h3 className="arcade-text text-yellow-500 text-xs mb-1">FULFILLER</h3>
                          <p className="arcade-text text-gray-300">
                            {truncateText(fulfillment.fulfiller, 10, 8)}
                            <button 
                              onClick={() => navigator.clipboard.writeText(fulfillment.fulfiller)}
                              className="ml-2 text-primary-500 text-xs hover:text-primary-400"
                              title="Copy full address"
                            >
                              [COPY]
                      ...
                      {fulfillment.tx_hash && (
                        <div>
                          <h3 className="arcade-text text-yellow-500 text-xs mb-1">TRANSACTION</h3>
                          <div className="flex items-center">
                            <p className="arcade-text text-gray-300">
                              {truncateText(fulfillment.tx_hash, 10, 8)}
                            </p>
                            <button 
                              onClick={() => navigator.clipboard.writeText(fulfillment.tx_hash)}
                              className="ml-2 text-primary-500 text-xs hover:text-primary-400"
                      ...
                      {fulfillment.amount && (
                        <div>
                          <h3 className="arcade-text text-yellow-500 text-xs mb-1">AMOUNT FULFILLED</h3>
                          <p className="arcade-text text-gray-300">{formatTokenAmount(fulfillment.amount, intent.token, intent.source_chain)}</p>
                        </div>
                      )}
                      
                      {fulfillment.created_at && (
                        <div>
                          <h3 className="arcade-text text-yellow-500 text-xs mb-1">FULFILLMENT TIME</h3>
                          <p className="arcade-text text-gray-300">{formatDate(fulfillment.created_at)}</p>
                        </div>
                      )}

                      {fulfillment.tx_hash && intent && intent.destination_chain && (
                        <div className="md:col-span-2 mt-2 text-center">
                          <a 
                            href={getExplorerUrl(parseInt(intent.destination_chain), fulfillment.tx_hash)}
                            target="_blank"
                          ...
                          >
                            VIEW TRANSACTION ON {getChainName(intent.destination_chain)} EXPLORER
                          </a>
                        </div>
                      )}
                    </div>
                ...
                <div className="border-t border-gray-700 pt-4 mt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {intent.created_at && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">CREATED AT</h3>
                        <p className="arcade-text text-gray-300">{formatDate(intent.created_at)}</p>
                      </div>
                    )}
                    
                    {intent.updated_at && (
                      <div>
                        <h3 className="arcade-text text-yellow-500 text-xs mb-1">UPDATED AT</h3>
                        <p className="arcade-text text-gray-300">{formatDate(intent.updated_at)}</p>
                      </div>
                    )}
                  </div>
                </div>
