...
export function useIntentForm() {
  ...
  const [intentStatus, setIntentStatus] = useState<"pending" | "fulfilled">(
    "pending",
  );
    ...
    const checkFulfillment = async () => {
      if (intentStatus === "fulfilled" && formState.intentId) {
        try {
          ...
          if (
            intent &&
            (intent.status === "fulfilled" || intent.status === "settled")
          ) {
            try {
              // Get the fulfillment data using the API service
              const fulfillment = await apiService.getFulfillment(
                formState.intentId,
              );
              ...
              console.error(
                "Error accessing fulfillment data:",
                fulfillmentError,
              );
